...
Coq/MathCompには、証明済みの``append``や``reverse``関数が用意されているので、
...
簡単な例として、リストを連結する``append``の証明をしてみます。
...
(**
Coqで扱い易いように、PrologのプログラムをHypothesisとして定義します。
*)
...
(**
Prologのプログラム``append``の第1引数と第2引数を連結（MathCompでは``++``）
したものが第3引数になることを証明します。
...
Module LRev.
  ...
  Hypothesis rv1 : forall X XS ACC RL, rev3 XS (X :: ACC) RL -> rev3 (X :: XS) ACC RL.
...
途中で、MathCompのrevと++関数について、``rev (a :: A) ++ B = rev A ++ (a :: B)`` 
...
といっても、appendを使ってreverseを定義した述語です。
...
Axiom srv0 : forall A B, srev3 A [::] B B -> srev A B.
Axiom srv1 : forall A, srev3 [::] A A [::].
Axiom srv2 : forall A B C D E F, srev3 A (B :: C) D F -> srev3 (B :: A) C D (E :: F).

Lemma srev_rev A : srev A (rev A).
Proof.
Admitted.
