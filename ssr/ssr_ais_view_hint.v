...
*)
  ...
  Proof.
    ...
    Undo 1.
    apply (iffLR (PQequiv a)) in HPa; apply HPa.
  Qed.
End Sample2.

(**
# 標準のView Hint (iffLRn iffRLn iffLR iffRL)

SSReflectのソースコードを見ると、theories/ssreflect.v の最後で宣言されています。
...
(* 次はゴール「a&&b->a/\b」に対して、「a&&b->a/\b」を適用しているので、一般的な例になっていない。要補足 *)
  Goal a && b -> a /\ b.
  Proof.
    apply/andP.
    Undo 1.
...
これをまとめると次のようになります。
``
assumption interpretation (move/)で使用できる：
elimTF elimNTF elimTFn
introT introTn introN

goal interpretations (apply/)で使用できる：
elimT elimTn elimN
introTF introNTF introTFn

right hand sides of double views (apply//)で使用できる：
equivPif equivPifn
xorPif xorPifn
``
*)
