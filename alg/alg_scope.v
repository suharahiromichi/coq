...
(**
```
Notation "x == y" := (eq_op x y) : bool_scope (default interpretation)
Notation "A == B" := (andb (submx.body A B) (submx.body B A)) : matrix_set_scope
Notation "x == y" := (GRing.Equal x y) : term_scope
```
*)
Locate "_ + _".
(**
...
(**
演算子に対して使いたいNotationが``(default interpretation)`` になっている場合、
とくに、Notationがひとつしかない場合は、その演算子の使用に問題はありません。

問題は、ひとつの演算子に対して複数のNotationがある場合です。
 *)
(**
## algebraの導入以前
...
これの影響は大きいため、乱用はせず、ring_scope のみに限定するのが習慣のようです。
*)
Open Scope ring_scope.
