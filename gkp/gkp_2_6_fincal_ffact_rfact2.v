...
この rfact_rec の定義は ffact_rec の定義に準じており、
いくつかの証明は ffact_rec と同じであるが、rfactSS の証明ができない。
 *)
Section RFACT.
  ...
  Proof.
    ...
    - admit.
    - rewrite mulnDr.
      rewrite [n.+2 ^^ m * n.+1]mulnC.
      rewrite -[n.+1 * n.+2 ^^ m]rfactnS.
      rewrite IHn.
  Admitted.
  
  Lemma rfactnSr n m : 0 < m -> n ^^ m.+1 = n^^m * (n + m).
  Proof.
    elim: n m => [|n IHn] m H.
    - rewrite add0n.
      rewrite rfact0n' //=.
