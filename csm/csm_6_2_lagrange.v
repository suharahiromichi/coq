...
(**
これに直前に証明した補題(partition_rcosets)を前提に与えます。

すると、剰余群のすべての要素（すべての剰余類）の濃度の総和は、
群$G$の濃度に等しい、となります。これは、剰余群は、群$G$の分割であるためですね。
...
(**
これを使って、最初に証明した補題 myCard_rcoset でΣの中を書き換えると次のようになります。
*)
  Check @myCard_rcoset G H : forall A : {set gT},
      A \in rcosets H G -> #|A| = #|H|.
  Check ((eq_bigr (fun _ => #|H|)) (@myCard_rcoset G H))
    : \sum_(A in rcosets H G) #|A| = \sum_(A in rcosets H G) #|H|.
