...
(*
verification_conditions において、DCWhileの <~~> を ~~> にした版。
オリジナルのコメントにある Alexandre Pilkiewicz の別案だと、
WHILEの終了条件をWHILEの後に持ち出せないないので、使い勝ってが悪い。
...
(* 
  発表用サンプルコード
 *)
Definition test_com_dec (a b : nat) : dcom :=
  (
    {{ fun st => asnat (st X) = a /\ asnat (st Y) = b }}
...
Theorem test_com_correct : forall (a b : nat),
  dec_correct (test_com_dec a b).
Proof.
  intros. verify.
