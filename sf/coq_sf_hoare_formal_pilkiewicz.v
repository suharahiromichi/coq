...
」
を反映した版である。
verification_correct までの変更箇所については、(A.P.)とコメントしてある。

実際の証明は、WHILE文自体のポスト条件がなくなるほか、証明責務が少なくなる。
*)
...
Inductive dcom : Type :=
  ...
  | DCWhile : bexp -> Assertion -> dcom -> dcom (* A.P. *)
...
Fixpoint extract (d:dcom) : com :=
  ...
  | DCWhile b _ d     => WHILE b DO extract d END (* A.P. *)
  | DCPre _ d         => extract d
...
Fixpoint verification_conditions (P : Assertion) (d:dcom) : Prop :=
  ...
  | DCWhile b P' c  =>                      (* A.P. *)
    (P ~~> post c)                          (* post c is the loop invariant *)
    /\ ((fun st => post c st /\ bassn b st) ~~> P')
    /\ verification_conditions (fun st => post c st /\ bassn b st) c
  | DCPre P' d         =>
...
Proof.
  ...
  Case "While".                             (* A.P. *)
    rename a into P'.
    inversion H as [Hpre [Hbody Hd]]. clear H.
...
(* 帰結の=>を左を抜くと、証明が立ち行かなくなる。

また、修飾の論理式をImpの論理式としてbassnで包むと、証明はより簡単になる。
しかし、それが証明になるとはいえないだろう。
 *)
