...
Section Recrel.
  ...
  Lemma mul2div2 (n : nat) : n.*2./2 = n.
  Proof.
  Admitted.
  
  Lemma test (n : nat) : n.+1 ^ 2 = n^2 + n.*2 + 1.
  Proof.
  Admitted.

  Lemma test2 (n : nat) : (n.+1).*2 = n.*2 + 2.
  Proof.
  Admitted.

  Lemma test3 (n : nat) : odd (n^2 + n) && odd (n.+1).*2 = 0 :> nat.
  Proof.
    (* 右が偶数なので、必ず成り立つ。 *)
  Admitted.
  
  Lemma test4 (n : nat) : n.+1^2 + n.+1 = (n^2 + n) + (n.+1).*2.
  Proof.
  Admitted.
