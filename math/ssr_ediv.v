...
整数 m, d, q, r に対して、与えられた m, q から d, r を求めること、と形式化できます。
...
(注2) m と d と q の符号は、偶数個の関係にあります。

剰余 r は式(1.2)から求めます。

このほか 減算を繰り返す方法もありますが、自然数の除算じたい減算で定義されるので
それに含めることにします。


## 今回使用する計算式

ここでは、剰余が式(1.3')を満たすものとし、
...
(**
自然数の文脈では ``eq nat``、整数の文脈では ``eq int`` であるのは当然ですが、
...
整数の文脈の中で、自然数を書くとコアーションで正整数になる（Posz これは ``%:Z`` と同じ）。

_%Nで自然数のスコープを明示することができるが、この外に出た時点で、
同様にコアーションで正整数になる。すなわち、``%:Z`` は分配できるわけである。
