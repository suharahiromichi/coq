...
ります。 通常、結合法則についての補題を使用した書き換えでは、ユーザーが等価関係
の左側と右側のどちらで起こるかを指定する必要があるため、``addrA`` の書き
換えを ``rewrite [in LHS]addrA`` に書き換える必要がある場合があります。
具体的な例については 4.4 を参照してください。 そのようなユーザーの場合、
このチュートリアルを最後まで読む必要はないかもしれません。
...
具体的にするために、セクション4 で CompDecModal [5] のポートを説明します。
その前に、セクション2 で HB の基本を確認します。
セクション3で移植に使用できるドキュメントとツールを確認します。 
 *)

(**
# 2. Quick Reminder about the HB Vocabulary 

このセクションの目的は、HB によって導入された 3 つの主要なコマンド、HB.mixin、HB.struction、
および HB.instance を簡単に説明することです。 知識のある読者はこのセクションを読み飛ばしても問題ありません。

最も基本的なシナリオを一般的な観点から考えてみましょう。 
以下は階層の最下位に位置する構造体 Struct を宣言するパターンです。 
構造体のインターフェイスは``Mixin``に入ります。
...
HB は Coq 属性を使用して、構造に対応する型を宣言していることに注意してください。 
以下は、既存の構造体 Struct を拡張する新しい構造体 NewStruct を宣言するパターンです。
注 ``of`` 構文に注意してください。
...
このプロセスの結果、型 structType と newStructType が作成され、
後者の要素は前者の要素としても理解されるようになります。
最後に、ミックスイン Struct の宣言には、コンストラクター Struct.Build が作成され、
次のコマンドを使用して構造体をインスタンス化するために使用されます。
...
たとえば、ファイル ssreflect/eqtype.v で定義されている eqType 構造を参照してください。
スクリプトのドキュメントの詳細については、この Wiki エントリを参照してください。

- オプションで、ユーザーは CONTRIBUTING.md [9] で説明されている命名規則に従って
識別子と補題の命名を再確認できます。

## 3.2 HB Commands Useful to Explore an Existing Hierarch

changelog と Coq スクリプトのヘッダーに加えて、
...
HB 階層のグラフ コマンド HB.graph を使用して HB 階層を調べることもできます。
Coq ファイル内:
...
eqType インスタンスは hasDecEq.Build で構築できることがわかります。 
(デフォルトでは、HB.howto は利用可能なすべてのファクトリーを返さない可能性があることに注意してください。
HB.howto xyzType 5 のように自然数を使用して深さの検索を増やす必要がある場合があります。)
xyz.Build コンストラクターがどのパラメーターを予期しているかを知るには、HB.about コマンドを使用できます。
...
このメッセージは、 hasDecEq.Build が型 T、述語 ``eq_op : rel T`` (角括弧で示される暗黙の引数)、
および ``Equality.axiom eq_op`` の証明を予期していることを示します。
したがって、ある型 T で eqType をインスタンス化するには
...
そのうちの数行が出力されるはずです (この出力がない場合は、
インスタンス化に問題があることを示していることが多いことを思い出してください)1:
...
HB.about でリストされている構造体とコンストラクターに加えて、
ライブラリではいくつかのエイリアス (別名フェザー ファクトリ) が定義されています。 
これらのエイリアスはヘッダーのコメントに記載されています。 
たとえば、ある型 T の eqType インスタンスは、
関数 ``f : T -> T'`` と証明 ``injf : injective f`` が与えられると、
すでに eqType 構造を備えたある T' から派生できます。
...
タイプにすでに装備されているインスタンスは、HB.about を使用してリストできます。
たとえば、次のようになります。
...
既存の MathComp 開発を MathComp 2 に移植する基本戦略は、
(1) Math-Comp 2 をインストールし、
(2) 既存の Coq スクリプトをコンパイルし、
(3) エラーを次々に修正することです。 
具体的にするために、CompDecModal [5] の移植について説明します。 

``https://github.com/coq-community/comp-dec-modal``

これは、MathComp を適度に使用した開発であり、
その移植には、MathComp を使用するほとんどの開発で使用される可能性が高い基本構造の
インスタンス化の修正が含まれます。
...
MathComp ユーザーの観点から見ると、主な変更点は数学的構造がインスタンス化される方法です。 
ほとんどの Canonical (または Canonical Structure) コマンドは HB.instance (セクション2 を参照) 
に置き換えられ、[subType ...] などの MathComp 表記に小さな変更が加えられています。
CompDecModal に関して、最初の問題となるコマンド セットは次のとおりです
(file ``theories/libs/fset.v``)。
...
このエラーは、変更ログに記載されている表記の変更が原因です。 たとえば、次のように文字列を検索します。
CHANGELOG.md の ``[subType`` :
...
インスタンスに名前を付ける必要はありません。
インスタンスを自動的に見つけるのは HB の仕事であるため、名前を付けないほうがよいことに注意してください。
HB が HB.instance への応答として複数のメッセージを表示することを確認することが重要です。
そうでない場合、これはインスタンス化の失敗を示している可能性があります。
次のコンパイル エラー:
...
このエラーは主に、EqType コンストラクターの削除が原因です [6、セクション 6]。 2.1]。
実際、MathComp のほとんどの xyzType コンストラクターはもう必要ありません。 changelog を参照してください。
上記の [subType for _ by _] notationと同様に、[eqMixin of _ by <:] は次のように変更されました。
...
HB は #[hnf] 属性を提供しますが、通常は必要ないことに注意してください。
最後に、最後の Canonical コマンドにより、対処する必要がある非推奨の警告が表示されます。
...
実際、1ステップ戻ると、HB.instance コマンドの出力から、Countable 構造体のインスタンス化が
すでに SubCountable 構造体のインスタンス化をトリガーしており、
最後の Canonical コマンドが有害になっていることがわかります。 
したがって、削除する必要があります。
要約すると、完全な修正は次のとおりです。
...
実際、さらに一歩先に進むことができます。
5行目で Equality 構造体をインスタンス化し、次に 7行目で Choice 構造体をインスタンス化する代わりに、
Choice 構造体をインスタンス化して Equality 構造体を自動的に取得することで、
より短い修正は次のようになります。
...
CompDecModal の移植時に発生する次の一連のコンパイル エラーは、
ほとんどの MathComp 開発で使用される可能性が高い、有限反復演算子に関するものです。
...
インスタンス化でキー (演算子 fsetU) を明示的にしたことに注意してください。
厳密に必要というわけではありませんが、この機会に文書化しておくことをお勧めします。
...
これは上記と似ています。Monoid.ComLaw について知るには HB.about を使用し、
その構築について調べるには HB.howto を使用します。
...
キー fsetU にはすでに Monoid.Law 構造が装備されているため、
このキーを追加パラメーターとして HB.howto に追加して、検索を制限できることに注意してください。
...
実際、Monoid.ComLaw 構造体のインスタンス化から Monoid.Law 構造体を取得できるため、
インスタンス化を 1つだけ行うこともできます。そのため、より良い修正は次のようになります。
...
この障害は主に EqMixin の削除が原因で発生します。
変更ログでは、パラメータを HB.howto で二重チェックできるコンストラクター hasDecEq.Build を
使用することを提案しており、これにより次の修正が行われます。
...
PcanCountMixin が非推奨となり、CountChoiceMixin が廃止されたことがわかりました。 
changelog では、PcanCountMixin の代わりに PCanIsCountable を使用することが推奨されています。
Locate によると、PCanIsCountable はファイル ``ssreflect/choice.v`` にあり、次のタイプを持っています。
...
インスタンス化を実行するときに推奨されるように、
キー (ここではフォーム) を明示するために型情報を追加したことに注意してください。
Countable インスタンスとともに Choice インスタンスも生成されるため、この修正で十分です。


#### ``Equality`` 構造体のその他のインスタンス化

ファイル ``Kstar_def.v``、``gen_def.v``、および ``CTL_def.v`` のなかの次のコンパイルエラーは、
すでに上で説明したように処理されます。

#### ``rewrite``タクティクの失敗

次のコンパイル「エラー」はファイル ``CTL/demo.v`` にあります。 
これは実際には、MathComp 2 へのアップグレードによって実行が大幅に遅くなった戦術です。

```coq:mathcomp1
move => [p' y]. rewrite /MRel /Mstate (negbTE (root_internal _)) [_ && _]/= orbF.
```

問題のある書き換えは orbF によるものです。 
以前は、デフォルトで ``<->`` 等価の左側に適用されていましたが、
現在では、適切な実行時間を回復するために、ユーザーはパターンで書き換え位置を指定する必要があります。
...
このドキュメントでは、典型的な MathComp 開発の MathComp 2 への移植について説明しました。
私たちは利用可能なツール (ドキュメントと HB 戦術) をレビューし、
多数の具体的なサンプル エラーと警告を調べて説明し、修正しました。
CompDecModal の移植例では、HB を使用すると読みやすさの点で Coq スクリプトが改善され、
さらには改善が可能になることが実証されました。 完全な修正はオンラインで見つけることができます。
編集には 10 個のファイル、35 個の挿入、67 個の削除が含まれており、これはおそらく中程度の作業量に相当します。
MathComp 2 への移行プロセスでは、
コミュニティが https://coq.zulipchat.com の math-comp ストリーム
を通じて回答できるさらに多くの質問が確実に生成されるでしょう。
