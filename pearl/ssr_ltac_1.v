...
``(a != b) <> false`` もあれば、``~~ (a != b) = false`` も可能という具合に、
百花繚乱の状態です。二重否定も普通に出現することも事態を複雑にします。

このことは Mathcomp の「等号問題」と呼ばれています（提唱者は私です）。
ここでの「等号」は、「EQUAL」と「NOT EQUAL」の両方を指しています。
...
この表だけで、便利につかえるかもしれせんね。

これらのタクティクをできるだけ繰り返して、もっとも簡単なかたちにします。
一般に、``a != b`` に ``move/negP`` を repeat で繰り返し適用すると、
``a != b`` と ``~ (a == b)`` の間を限りなく往復しててタクティクスが終了しません。
