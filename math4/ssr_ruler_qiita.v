...
を使います。``.-`` は ldiff 関数 であり、ビット反転したものとの論理和をとるビット演算のひとつで、
Standard Rocqに用意されています。ldiffは 自然数で定義されているのでて、
全体が自然数の範囲で定義できるので「自然数化した式」と呼びます。
...
(**
なお、自然数の範囲では、全ビットを反転する lnot 関数 ``.~ ○`` は定義されていない
...
注3 ビット反転 lnot は、自然数の範囲で定義できないので、ここは直感的な説明です。
*)
