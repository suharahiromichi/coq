(**
「数学ガールの秘密ノート／ビットとバイナリ」のルーラー関数とそのの漸化式の証明
=========================
...
扱う数の範囲が整数になり難しいので、自然数の範囲で、それと同値の定義である

``log2 (n .- (n - 1))`` ただし ``.-`` は ldiff 関数


を使います。これを自然数化した式と呼びます。

漸化式は自然数の範囲で定義されているので、両者の同値の証明は簡単になります。
...
また、p 関数だけでは証明が進まないため、testbit関数を使用します。

``x.[i]``


x の i 桁目の値が 0 なら false、1 なら true を返す。
